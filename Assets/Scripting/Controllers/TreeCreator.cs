...
public class TreeCreator : MonoBehaviour 
{
    ...
    void Update()
    {
        ...
        {
            if (Vector3.Distance(_car.position, _trees[i].position) > 300)
            {
                d.Add(i);
    ...
    private void CreateRandomTree()
    {
        ...
        Vector3 pos = new Vector3(Random.Range(20f, 80f) * Mathf.Sign(Random.value - 0.5f), 0, Random.Range(0f, 300f) * Mathf.Sign(Random.value - 0.5f));
    ...
    private Transform CreateForwardTree()
    {
        ...
        pos.x = Random.Range(20f, 80f) * Mathf.Sign(Random.value - 0.5f);
